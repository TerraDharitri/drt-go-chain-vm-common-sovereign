...
func (e *dcdtDataStorage) SaveDCDTNFTToken(
	...
	dcdtIdentifier, err := getDcdtIdentifierWithoutBaseKeyPrefix(dcdtTokenKey)
	if err != nil {
		return nil, err
	}

	if !e.crossChainTokenCheckerHandler.IsCrossChainOperation(dcdtIdentifier) {
...
func getDcdtIdentifierWithoutBaseKeyPrefix(dcdtTokenKey []byte) ([]byte, error) {
	if len(dcdtTokenKey) < len(baseDCDTKeyPrefix) {
		return nil, fmt.Errorf("invalid dcdt token key %s", string(dcdtTokenKey))
	}

	return dcdtTokenKey[len(baseDCDTKeyPrefix):], nil
}
