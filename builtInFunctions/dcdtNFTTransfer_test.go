...
func createNFTTransferAndStorageHandler(selfShard, numShards uint32, globalSettingsHandler vmcommon.GlobalMetadataHandler, enableEpochsHandler vmcommon.EnableEpochsHandler) (*dcdtNFTTransfer, *dcdtDataStorage) {
	...
	dcdtStorageHandler := createNewDCDTDataStorageHandlerWithArgs(globalSettingsHandler, accounts, enableEpochsHandler, &mock.CrossChainTokenCheckerMock{})
	nftTransfer, _ := NewDCDTNFTTransferFunc(
		1,
		marshaller,
...
func createNftTransferWithMockArguments(selfShard uint32, numShards uint32, globalSettingsHandler vmcommon.GlobalMetadataHandler) *dcdtNFTTransfer {
...
func TestDcdtNFTTransfer_SenderHasDynamicRoleAndLiqudityReaches0AfterTransfer_ShouldNotDeleteMetadata(t *testing.T) {
	dynamicRoles := [][]byte{
	...
	for _, role := range dynamicRoles {
		testName := fmt.Sprintf("sender has %s role", role)
		t.Run(testName, func(t *testing.T) {
			senderHasDynamicRoleAndLiqudityReaches0AfterTransfer(t, role)
		})
	}
}

func senderHasDynamicRoleAndLiqudityReaches0AfterTransfer(t *testing.T, role []byte) {
	t.Parallel()
	...
	nftTransferSenderShard, _ := createNFTTransferAndStorageHandler(
		...
		&mock.EnableEpochsHandlerStub{
			IsFlagEnabledCalled: func(flag core.EnableEpochFlag) bool {
				return flag == CheckTransferFlag || flag == CheckFrozenCollectionFlag || flag == SendAlwaysFlag || flag == SaveToSystemAccountFlag || flag == DynamicDcdtFlag
			},
		})
	...
	createSFT(tokenName, tokenNonce, nftTransferSenderShard, sender.(vmcommon.UserAccountHandler), role)
	...
	nonceBytes := big.NewInt(int64(tokenNonce)).Bytes()
	quantityBytes := big.NewInt(3).Bytes()
	vmInput := &vmcommon.ContractCallInput{
		VMInput: vmcommon.VMInput{
			CallValue:   big.NewInt(0),
			CallerAddr:  senderAddress,
	...
	systemSCAccount, err := nftTransferSenderShard.accounts.LoadAccount(vmcommon.SystemAccountAddress)
	require.Nil(t, err)

	tokenId := append(keyPrefix, tokenName...)
	dcdtNFTTokenKey := computeDCDTNFTTokenKey(tokenId, tokenNonce)
	dcdtData := &dcdt.DCDigitalToken{Value: big.NewInt(0), Type: uint32(core.Fungible)}
	marshaledData, _, err := systemSCAccount.(vmcommon.UserAccountHandler).AccountDataHandler().RetrieveValue(dcdtNFTTokenKey)
	require.Nil(t, err)
	require.True(t, len(marshaledData) > 0)
	_ = nftTransferSenderShard.marshaller.Unmarshal(dcdtData, marshaledData)
	assert.Equal(t, big.NewInt(0), dcdtData.Value)
	assert.Equal(t, []byte("NFT hash"), dcdtData.TokenMetaData.Hash)
	assert.Equal(t, uint64(1), dcdtData.TokenMetaData.Nonce)
	assert.Equal(t, [][]byte{[]byte("uri")}, dcdtData.TokenMetaData.URIs)
}

func createSFT(tokenName []byte, tokenNonce uint64, nftTransferSenderShard *dcdtNFTTransfer, sender vmcommon.AccountHandler, role []byte) {
	initialTokens := big.NewInt(3)
	tokenId := append(keyPrefix, tokenName...)
	dcdtNFTTokenKey := computeDCDTNFTTokenKey(tokenId, tokenNonce)
	dcdtData := &dcdt.DCDigitalToken{
	...
	buff, _ := nftTransferSenderShard.marshaller.Marshal(dcdtData)
	_ = sender.(vmcommon.UserAccountHandler).AccountDataHandler().SaveKeyValue(dcdtNFTTokenKey, buff)

	roles := &dcdt.DCDTRoles{Roles: make([][]byte, 0)}
	roles.Roles = append(roles.Roles, role)
	rolesBuff, _ := nftTransferSenderShard.marshaller.Marshal(roles)
	roleKey := append(roleKeyPrefix, tokenName...)
	_ = sender.(vmcommon.UserAccountHandler).AccountDataHandler().SaveKeyValue(roleKey, rolesBuff)
	...
	systemSCAccount, _ := nftTransferSenderShard.accounts.LoadAccount(vmcommon.SystemAccountAddress)

	_ = systemSCAccount.(vmcommon.UserAccountHandler).AccountDataHandler().SaveKeyValue(dcdtNFTTokenKey, buff)
	_ = nftTransferSenderShard.accounts.SaveAccount(systemSCAccount)
}

func TestDCDTNFTTransfer_ProcessBuiltinFunctionOnSovereignTransfer(t *testing.T) {
	transferFunc := createNftTransferWithMockArguments(0, 1, &mock.GlobalSettingsHandlerStub{})
	_ = transferFunc.SetPayableChecker(&mock.PayableHandlerStub{})
	...
	dcdtStorage := createNewDCDTDataStorageHandlerWithArgs(transferFunc.globalSettingsHandler, transferFunc.accounts, enableEpochsHandler, &mock.CrossChainTokenCheckerMock{})
	transferFunc.dcdtStorageHandler = dcdtStorage
	...
	dcdtDataBytes := createDCDTNFTTokenWithReservedField(token1, core.NonFungible, token1Nonce, sysAccNFTInitialQuantity, transferFunc.marshaller, sysAcc.(vmcommon.UserAccountHandler), reserved)

	vmInput := &vmcommon.ContractCallInput{
		VMInput: vmcommon.VMInput{
			CallValue:   big.NewInt(0),
			CallerAddr:  senderAddress,
			Arguments:   [][]byte{token1, nonce1Bytes, sysAccNFTInitialQuantity.Bytes(), dcdtDataBytes},
			GasProvided: 0,
		},
		RecipientAddr: destinationAddress,
	}
